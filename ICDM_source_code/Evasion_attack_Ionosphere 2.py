...
X = pd.read_csv('ionosphere.data', header=None)
print(X.shape)
X2 = np.array(X.loc[:, :])
...
for top_k in number_of_list:
    for t in range(len(dmax)):
        ...
        print("----------------------------------------------------------------------------------------------------------")
        for i in range(m):
            subsample_x_array = np.zeros([iter_n, 34], dtype=float)
            subsample_y_array = []
            # rf = RandomForestRegressor()
            # rf_adv = RandomForestRegressor()
            # rf = Ridge(alpha=0.5)
            # rf_adv = Ridge(alpha=0.5)
            # rf = linear_model.Lasso(alpha=0.05)
            # rf_adv = linear_model.Lasso(alpha=0.05)
            ...
            pgd_ls_attack = CAttackEvasionPGDLS(classifier=clf, double_init_ds=train_subsample_, double_init=False,  dmax = dmax[t], lb=lb, ub=ub, solver_params=solver_params, y_target=y_target)
            # 对抗训练特征选择
