...
# X = pd.read_csv('csv/madelon.csv', header=None)
X = pd.read_csv('madelon_train.data', delim_whitespace=True,header=None)
y = pd.read_csv('madelon_train.labels', header=None)
print(X)
print(X.shape)
# X2 = np.array(X.loc[:, :])
...
for t in range(len(dmax)):
    ...
    for i in range(m):
        ...
        # rf = linear_model.Lasso(alpha=0.01)
        # rf_adv = linear_model.Lasso(alpha=0.01)
        train_subsample = random.sample(list(train), 100)
        X = np.array(train_subsample)[:, :-1]
        y = np.array(train_subsample)[:, -1]
        # print(X.shape)
        # print(y.shape)
        ...
        # mrmr
        # selected_features = mrmr_classif(X_adv, y, K=100)
