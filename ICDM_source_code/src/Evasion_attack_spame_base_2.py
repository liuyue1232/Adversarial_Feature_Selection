...
number_of_adv_samples = [50, 100, 150, 200]
...
for num in number_of_adv_samples:
    acc_fs_rf = []
    acc_fs_rf_adv = []
    acc_fs_rf_adv_combine = []
    for t in range(len(dmax)):
        ...
        for i in range(m):
            ...
            # rf = Ridge(alpha=1.8)
            # rf_adv = Ridge(alpha=1.8)
            rf = linear_model.Lasso(alpha=0.04)
            rf_adv = linear_model.Lasso(alpha=0.04)
            # rf = linear_model.Lasso(alpha=0.01)
            # rf_adv = linear_model.Lasso(alpha=0.01)
            train_subsample = random.sample(list(train), 200)
            ...
            # # 选择20个特征
            RFselec = [i[0] for i in score_sorted][: 20]
            RFselec.sort()
            # mrMR
            # selected_features = mrmr_classif(X, y, K = num)
            # RFselec = selected_features
            # RFselec.sort()
            print("num: ", num, "RFselec:", RFselec)
            # 没有逃避攻击
            ...
            # print(y_adv.shape)
            # selected_features = mrmr_classif(X_adv, y, K=num)
            # RFselec_adv = selected_features

            row_total = X_adv.shape[0]
            row_sequence = np.arange(row_total)
            np.random.shuffle(row_sequence)
            X_adv_combine_ = X_adv[row_sequence[0:num], :]
            y0 = y
            y_combine_ = y0[row_sequence[0:num]]

            X_adv_combine = np.concatenate((X_adv_combine_, X), axis = 0)
            y_adv_combine = np.concatenate((y_adv, y), axis = 0)
            y_combine = np.concatenate((y_combine_, y0), axis = 0)
            # print(X_adv_combine.shape)
            # print(y_combine.shape)
            # mrmr
            # selected_features_combine = mrmr_classif(X_adv_combine, y_combine, K=num)
            ...
            score_sorted = sorted(score_inial.items(), key=lambda x: x[1], reverse=True)  # RF选出的特征重要性分数 , RFE的reverse=False
            # 选择20个特征
            RFselec_adv = [i[0] for i in score_sorted][: 20]
            ...
            score_sorted = sorted(score_inial.items(), key=lambda x: x[1], reverse=True)  # RF选出的特征重要性分数 , RFE的reverse=False
            # 选择20个特征
            RFselec_adv_combine = [i[0] for i in score_sorted][: 20]

            RFselec_adv.sort()
            RFselec_adv_combine.sort()
            print("num: ", num, "RFselec_adv_combine:", RFselec_adv_combine)
            print("num: ", num, "RFselec_adv:", RFselec_adv)

            # n正常样本 + n对抗样本
            X_train_feature_selection_rf_combine = X_adv_combine[:, RFselec_adv_combine]
            clf_fs_rf_combine = svm.SVC(kernel='linear', C=1).fit(X_train_feature_selection_rf_combine, y_combine.astype('int'))
            ...
            for j in range(d):
                if j in RFselec_adv_combine:
                    Z_after_adv_combine[i, j] = 1
    ...
    sum = 0
    for i in range(len(acc_fs_rf)):
        sum += acc_fs_rf[i]
    acc_fs_rf_avg.append(sum/m)
    
    sum = 0
    for i in range(len(acc_fs_rf_adv_combine)):
        sum += acc_fs_rf_adv_combine[i]
    acc_fs_rf_adv_combine_avg.append(sum/m)
    
    sum = 0
    for i in range(len(acc_fs_rf_adv)):
        sum += acc_fs_rf_adv[i]
    acc_fs_rf_adv_avg.append(sum/m)

print(acc_fs_rf_avg)
print(acc_fs_rf_adv_combine_avg)
