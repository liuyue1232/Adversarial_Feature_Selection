...
# clf_svc = svm.SVC(kernel='rbf', C=1, gamma='auto').fit(X_train, y_train.astype('int'))
...
# for i in range(m):
#     train_subsample = random.sample(list(train), 30)
#     # print(np.array(train_subsample).shape)
#     X = np.array(train_subsample)[:, :-1]
#     y = np.array(train_subsample)[:, -1]
#     rf.fit(X, y)
#     w_randomForest = rf.feature_importances_
...
#
# print(Z)
# stab_random_forest = st.getStability(Z)
...
lb, ub = 0, 10  # Bounds of the attack space. Can be set to `None` for unbounded
...
for t in range(len(dmax)):
    ...
    print(adv_ds_pgdls.X)
    print(adv_ds_pgdls.Y)
    print("dmax = ",t)
    print("----------------------------------------------------------------------------------------------------------")
    for i in range(m):
        ...
        # selector = RFE(estimator, n_features_to_select=2000, step=1)
        ...
        score_inial_adv = dict(map(lambda x, y: [x, y], key, w_randomForest_adv))
        score_sorted_adv = sorted(score_inial_adv.items(), key=lambda x: x[1], reverse=True)  # RF选出的特征重要性分数 , RFE的reverse=False
        # 选择20个特征
        top_k = 2000
        RFselec_adv = [i[0] for i in score_sorted_adv][: top_k]
        RFselec_adv.sort()
        # print("RFselec_adv:", RFselec_adv)

        # 没有逃避攻击
        X_train_feature_selection_rf_adv = X_adv[:, RFselec_adv]
        # X_test_feature_selection_rf = X_test[:, RFselec_adv]
        clf_fs_rf_adv = svm.SVC(kernel='linear', C=1).fit(X_train_feature_selection_rf_adv, y.astype('int'))
        # y_predict_feature_selection_rf = clf_fs_rf.predict(X_test_feature_selection_rf)
        # # print("f1_RF特征选择后:", f1_score(y_predict_feature_selection_rf, y_test.astype('int'), average='macro'))  # F1值一般越大越好
        ...
        y_predict_feature_selection_pgd_rf = clf_fs_rf_adv.predict(test_x_selected_features_pgd_rf)
        y_pred_pgdls = CArray(y_predict_feature_selection_pgd_rf)
