...
X = pd.read_csv('parkinsons.data', header=None)
# print(y)
print(X.shape)
X2 = np.array(X.loc[1:, 1:])
column_headers = list(X.columns.values)
# print("column:", column_headers)
X = np.delete(X2, -7, axis=1)
print(X.shape)
print(X)
y = X2[:, -7]
y = y.astype(int)
...
for t in range(len(dmax)):
    ...
    print("----------------------------------------------------------------------------------------------------------")
    for i in range(m):
        subsample_x_array = np.zeros([iter_n, 22], dtype=float)
        subsample_y_array = []
        # rf = RandomForestRegressor()
        # rf_adv = RandomForestRegressor()
        # rf = Ridge(alpha=0.5)
        # rf_adv = Ridge(alpha=0.5)
        # rf = linear_model.Lasso(alpha=0.005)
        # rf_adv = linear_model.Lasso(alpha=0.005)
        ...
        # mrmr
        # selected_features = mrmr_classif(X_adv, y, K = 10)
