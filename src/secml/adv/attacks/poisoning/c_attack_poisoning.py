...
class CAttackPoisoning(CAttackMixin, metaclass=ABCMeta):
    ...
    def run(self, x, y, ds_init=None, max_iter=16):
        ...
        print(xc.tondarray().shape,yc.tondarray().shape)
        ...
        while delta > 0 and k < max_iter:
            ...
            print("Iter on all the poisoning samples: {:}".format(k))
            xc_prv = xc.deepcopy()
            for i in range(self._n_points):
                ...
                self.logger.info(
                    "poisoning point {:} optim fopt: {:}".format(
                        i, self._f_opt))
                print("poisoning point {:} optim fopt: {:}".format(i, self._f_opt))
                ...
                print("Poisoned classifier accuracy on test data {:}".format(acc))
            ...
            print("Optimization with n points: " + str(self._n_points) +
                " iter: " + str(k) + ", delta: " +
                str(delta) + ", fopt: " + str(self._f_opt))
            k += 1
